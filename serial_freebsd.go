...
func openPort(name string, baud int, databits byte, parity Parity, stopbits StopBits, readTimeout time.Duration) (p *Port, err error) {
	...
	f, err := os.OpenFile(name, syscall.O_RDWR|syscall.O_NOCTTY|syscall.O_NONBLOCK|syscall.O_EXCL, 0666)
	if err != nil {
		return nil, err
	}
	...
	t := syscall.Termios{
		...
		Cflag:  cflagToUse,
		Cc:     [20]uint8{syscall.VMIN: vmin, syscall.VTIME: vtime},
		Ispeed: rate,
		Ospeed: rate,
	}

	// from https://fossies.org/linux/cdrtools/include/schily/termios.h
	// in FreeBSD TCSETS = syscall.TIOCSETAW
	if _, _, errno := syscall.Syscall6(
		syscall.SYS_IOCTL,
		uintptr(fd),
		uintptr(syscall.TIOCSETAW),
